...
- **全程中文**：无论何种场景，AI 助手面向用户的所有自然语言输出一律使用中文，**没有例外场景**。具体覆盖（含但不限于）：
  - 纯对话问答、解释说明、方案讨论；
  - 计划模式（plan mode）下的计划、待办、阶段说明；
  - 任务执行过程中的每一步：开始前的说明、进行中的进度更新、工具调用前后的提示、遇到阻塞时的反馈、结束时的总结；
  - 提交信息（commit message）、PR 描述、代码评审意见、错误与异常的解释。
- **唯一允许英文的情形**：代码标识符（变量/函数/类名等）、文件路径、命令、第三方库/技术名词、API 字段名等「本就以英文存在」的内容保持原文，但**包裹它们的叙述句子仍用中文**。
- 即便用户用英文提问，回答仍默认用中文（除非用户明确要求改用其他语言）。
...
2. **前端在容器内多阶段构建**：`frontend/Dockerfile` 用 `node:20-alpine` 构建（`npm ci && npm run build`）→ `nginx:alpine` 托管产物，由 `docker-compose.yml` 的 `frontend` 服务 `build: ./frontend` 引用。构建在**测试机 Docker（Linux）内**进行，平台一致，故不存在 Windows↔Linux 的 optional 原生依赖 lock 漂移问题（早期曾因本机构建上传 dist 而踩过）。**`frontend/dist/` 不入库、不手动上传**；前端改动随 `up -d --build` 自动重建。本机仍可 `npm run build` 仅做编译自测。
...
### 测试机（Docker，dev）
```bash
# 拉取最新代码后，一条命令构建并启动（前后端均在容器内构建，dist 不需手传）
git pull
docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d --build
# 前端 :8088，后端 :8080（根路径返回 401 属正常，开放接口是 /api/v1/ping）
# 仅前端改动也走 --build（容器内重新 npm build）；改 nginx.conf 同理（已 COPY 进镜像）
```

**完成任意后端改动后，至少在本机跑通 `mvn -s settings.xml compile` 再提交；前端改动跑通 `npm run build` 再提交。**

### 验证-提交-部署闭环（务必遵守）
本机无 Docker、**无法运行后端**（只能编译验证，不能起服务做运行时验证）。因此运行时验证一律在测试机进行，标准闭环：
1. 代码编写完成 → 本机编译自测（后端 `mvn compile`、前端 `npm run build`）通过；
2. **提交并推送**（提交/推送前仍需按第 7 节自查范围并获用户确认）；
3. 测试机 `git pull` 部署运行（见上「测试机（Docker，dev）」），由**用户**做运行时验证；
4. **验证通过后用户会明确告知**——在此之前不要预先标记完成；
5. 收到用户验证通过的反馈后，再更新 [开发进度.md](开发进度.md)。

> 即：**本机编译 → 推送 → 测试机部署 → 用户验证 → 用户确认通过 → 改开发进度**。开发进度的更新以「用户确认测试机验证通过」为准，不以「本机编译通过」或「代码写完」为准。
...
开发计划与实时进度统一维护在 **[开发进度.md](开发进度.md)**（单一事实来源）。开始任何开发前先读该文件确认所处阶段与任务拆解；**待用户确认测试机验证通过后**再更新它（见第 6 节「验证-提交-部署闭环」），**无需改动本 CLAUDE.md**。
